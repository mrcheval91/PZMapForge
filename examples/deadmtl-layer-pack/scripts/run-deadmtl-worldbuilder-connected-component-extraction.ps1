...
# Splits source mask regions into 4-way connected pixel components.
...
# Layout not materialized. Connected component extraction only.
...
$outDir  = Join-Path $repoRoot ".local\deadmtl-authoring\worldbuilder-connected-component-extraction\map_00"
$outJson = Join-Path $outDir "map_00.connected_component_extraction.json"
$outMd   = Join-Path $outDir "map_00.connected_component_extraction.md"
$outCsv  = Join-Path $outDir "map_00.connected_component_extraction.csv"
$outSum  = Join-Path $outDir "map_00.connected_component_extraction.summary.txt"
...
if ($LASTEXITCODE -ne 0) {
    Write-Error "deadmtl-build-worldbuilder-connected-component-extraction failed (exit $LASTEXITCODE)"
    exit $LASTEXITCODE
}
