...
<#
.SYNOPSIS
MAP-34A: Controlled in-game load test installer for the MAP-33A binary-seeded runtime candidate.

Parameters:
  -InstallOnly  (default) Install candidate into local Zomboid mods folder and print manual test steps.
  -CollectLogs  Collect PZ console/debug logs after a manual test run.
...
if (-not (Test-Path $Map33AManifest)) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Error "MAP-33A helper failed (exit $LASTEXITCODE). Cannot continue MAP-34A."
        exit 1
    }
}
...
$collectFlag   = if ($isCollectLogs) { "--collect-logs" } else { "" }
...
Write-Host ""
Write-Host "=== MAP-34A In-Game Load Test ==="
...
if (-not $isCollectLogs) {
    ...
    Write-Host "8. Re-run this helper with -CollectLogs:"
    Write-Host "   powershell -ExecutionPolicy Bypass -File run-deadmtl-worldbuilder-map33a-ingame-load-test.ps1 -CollectLogs"
}

if ($isCollectLogs -and (Test-Path $OutputResult)) {
    $result = Get-Content $OutputResult -Raw | ConvertFrom-Json
    Write-Host ""
    Write-Host "=== Log Collection Results ==="
    Write-Host "Logs found           : $($result.runtime_logs_found)"
    Write-Host "Log count            : $($result.runtime_log_paths.Count)"
    Write-Host "Classification       : $($result.runtime_classification)"
    if ($result.runtime_classification -eq "MAP34A_RUNTIME_LOAD_PASS") {
        Write-Host "RESULT: Candidate appears to have loaded."
        Write-Host "NOTE: runtime_proof_claimed remains FALSE until operator confirms in-game evidence."
    } elseif ($result.runtime_classification -eq "MAP34A_RUNTIME_LOAD_FAIL") {
        Write-Host "RESULT: Errors detected in logs related to candidate."
    } else {
        Write-Host "RESULT: Evidence insufficient - logs do not conclusively prove load or failure."
    }
}
