...
MAP-27G consumes 1 MAP-27F acceptance gate result JSON + 7 MAP-27C replay source files,
...
`replay_lock_status: LOCKED_FOR_NEXT_SANDBOX_EXPERIMENT_ONLY`. 8 locked files: 1 MAP-27F acceptance gate result JSON + 7 MAP-27C replay source files.
`sandbox_only: true`, `accepted_for_runtime_writer: false`, `accepted_for_playable_export: false`,
`writer_ready: false`, `materialized: false`.
...
Key checks: `MAP27G_REPLAY_LOCK_ROOT_EXISTS`, `MAP27G_VERDICT_COMPLETE`, `MAP27G_IS_VALID_TRUE`,
`MAP27G_STATUS_LOCKED_FOR_NEXT_SANDBOX_EXPERIMENT_ONLY`, `MAP27G_REPLAY_LOCK_FILE_COUNT_8`,
`LOCKED_FILE_ROLES_EXACT_ORDER`, `LOCKED_FILE_1_ACCEPTANCE_GATE_RESULT_JSON_EXISTS` through
`LOCKED_FILE_8_MATERIALIZER_FORBIDDEN_OUTPUT_GUARD_JSON_EXISTS`, `ALL_8_LOCKED_FILES_HASHED`,
`ALL_8_LOCKED_FILE_HASHES_MATCH`, `REPLAY_LOCK_ID_RECOMPUTED`, `REPLAY_LOCK_ID_MATCHES_STORED`,
`NO_OLD_MAP27F_MD_CSV_SUMMARY_LOCKED`, `ALL_8_LOCKED_FOR_REPLAY_TRUE`,
`SANDBOX_MATERIALIZED_SOURCE_TRUE`, `VISUAL_QA_OVERLAY_WRITTEN_TRUE`, `PZ_RUNTIME_MATERIALIZED_FALSE`,
`MATERIALIZED_CELL_COUNT_5340`, `RENDERED_CELL_COUNT_5340`, `COUNT_MATCH_SUMMARY_MATCH`,
`WALL_COUNT_850`, `FLOOR_COUNT_2444`, `ACCESS_COUNT_148`, `LOT_COUNT_1898`,
`COMPONENT_RESIDUAL_COUNT_0`, `MATERIAL_KIND_COUNT_5`, `LAYER_KIND_COUNT_5`,
`NEXT_ALLOWED_EXPERIMENT_SANDBOX_ONLY`, `FORBIDDEN_STEPS_LISTED`, `POST_AUDIT_FORBIDDEN_SCAN_PASS`,
`WRITER_READY_FALSE`, `NO_RUNTIME_PROOF_CLAIM`, `NO_PUBLIC_PLAYABLE_PACKAGING_CLAIM`.
