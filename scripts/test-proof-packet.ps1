...
<#
.SYNOPSIS
    Validates .local/mapforge/proof-packet.json against the v0.16 proof-packet contract.
...
Assert-True ($p.schema -eq 'pzmapforge.proof-packet.v0.16') `
    "schema == 'pzmapforge.proof-packet.v0.16' (got '$($p.schema)')"
...
Assert-True ([int]$p.validation_summary.build42_geometry_inspector_tests  -eq 10)  "build42_geometry_inspector_tests == 10"
Assert-True ([int]$p.validation_summary.total_expected_assertions         -eq 502) "total_expected_assertions == 502"
...
Assert-True ([int]$d.test_total                                -eq 440)  "dotnet test_total == 440"
Assert-True ([int]$d.core_tests -eq 190)  "dotnet core_tests == 190"
Assert-True ([int]$d.cli_tests                                 -eq 250)  "dotnet cli_tests == 250"
