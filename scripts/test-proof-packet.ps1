...
<#
.SYNOPSIS
    Validates .local/mapforge/proof-packet.json against the v0.39 proof-packet contract.
...
Assert-True ($p.schema -eq 'pzmapforge.proof-packet.v0.39') `
    "schema == 'pzmapforge.proof-packet.v0.39' (got '$($p.schema)')"
...
Assert-True ([int]$p.validation_summary.total_expected_assertions              -eq 911) "total_expected_assertions == 911"
