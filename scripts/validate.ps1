...
.SYNOPSIS
    ...
    Final output reports the complete PowerShell validation lane total (2150)
    and the .NET lane total (664) as separate evidence lanes.
    Counts are sourced from proof-packet v0.83 / docs/VALIDATION_LEDGER.md.
...
Write-Output ""
Write-Output "--- MAP-38B differential control test packet (renderable_v1) ---"
$map38bDoc        = Join-Path $repoRoot 'docs\MAP_38B_DIFFERENTIAL_CONTROL_TEST.md'
$map38bPrepScript = Join-Path $repoRoot 'scripts\prepare-build42-map38b-differential-control-test-packet.ps1'
$map38bTests      = Join-Path $repoRoot 'scripts\test-build42-map38b-differential-control-test-packet.ps1'
if (-not (Test-Path -LiteralPath $map38bDoc))        { throw "MAP-38B doc missing" }
Write-Output "OK: docs\MAP_38B_DIFFERENTIAL_CONTROL_TEST.md"
if (-not (Test-Path -LiteralPath $map38bPrepScript)) { throw "MAP-38B prepare script missing" }
Write-Output "OK: scripts\prepare-build42-map38b-differential-control-test-packet.ps1"
if (-not (Test-Path -LiteralPath $map38bTests))      { throw "MAP-38B tests missing" }
Write-Output "OK: scripts\test-build42-map38b-differential-control-test-packet.ps1"
$map38bDocContent  = Get-Content -LiteralPath $map38bDoc -Raw
if ($map38bDocContent -notmatch 'MAP38B_DIFFERENTIAL_CONTROL_TEST_PACKET_DEFINED') { throw "MAP-38B doc missing MAP38B_DIFFERENTIAL_CONTROL_TEST_PACKET_DEFINED" }
Write-Output "OK: doc contains MAP38B_DIFFERENTIAL_CONTROL_TEST_PACKET_DEFINED"
if ($map38bDocContent -notmatch 'PLAYABLE_EXPORT_CLAIM_ALLOWED=false') { throw "MAP-38B doc missing PLAYABLE_EXPORT_CLAIM_ALLOWED=false" }
Write-Output "OK: doc contains PLAYABLE_EXPORT_CLAIM_ALLOWED=false"
if ($map38bDocContent -notmatch 'FALLBACK_INDISTINGUISHABLE') { throw "MAP-38B doc missing FALLBACK_INDISTINGUISHABLE" }
Write-Output "OK: doc contains FALLBACK_INDISTINGUISHABLE"
$map38bPrepContent = Get-Content -LiteralPath $map38bPrepScript -Raw
if ($map38bPrepContent -notmatch '\.local') { throw "MAP-38B prepare script missing .local refusal" }
...
& powershell -ExecutionPolicy Bypass -File $map38bTests
if ($LASTEXITCODE -ne 0) { throw "MAP-38B differential control test packet tests failed." }

Write-Output ""
Write-Output "--- MAP-37E differential control test packet ---"
$map37eDoc        = Join-Path $repoRoot 'docs\MAP_37E_DIFFERENTIAL_CONTROL_TEST.md'
$map37ePrepScript = Join-Path $repoRoot 'scripts\prepare-build42-map37e-differential-control-test-packet.ps1'
$map37eTests      = Join-Path $repoRoot 'scripts\test-build42-map37e-differential-control-test-packet.ps1'
if (-not (Test-Path -LiteralPath $map37eDoc))        { throw "MAP-37E doc missing" }
Write-Output "OK: docs\MAP_37E_DIFFERENTIAL_CONTROL_TEST.md"
if (-not (Test-Path -LiteralPath $map37ePrepScript)) { throw "MAP-37E prepare script missing" }
Write-Output "OK: scripts\prepare-build42-map37e-differential-control-test-packet.ps1"
if (-not (Test-Path -LiteralPath $map37eTests))      { throw "MAP-37E tests missing" }
Write-Output "OK: scripts\test-build42-map37e-differential-control-test-packet.ps1"
$map37eDocContent  = Get-Content -LiteralPath $map37eDoc -Raw
if ($map37eDocContent -notmatch 'MAP37E_DIFFERENTIAL_CONTROL_TEST_PACKET_DEFINED') { throw "MAP-37E doc missing MAP37E_DIFFERENTIAL_CONTROL_TEST_PACKET_DEFINED" }
Write-Output "OK: doc contains MAP37E_DIFFERENTIAL_CONTROL_TEST_PACKET_DEFINED"
if ($map37eDocContent -notmatch 'PLAYABLE_EXPORT_CLAIM_ALLOWED=false') { throw "MAP-37E doc missing PLAYABLE_EXPORT_CLAIM_ALLOWED=false" }
Write-Output "OK: doc contains PLAYABLE_EXPORT_CLAIM_ALLOWED=false"
if ($map37eDocContent -notmatch 'FALLBACK_INDISTINGUISHABLE') { throw "MAP-37E doc missing FALLBACK_INDISTINGUISHABLE" }
Write-Output "OK: doc contains FALLBACK_INDISTINGUISHABLE"
$map37ePrepContent = Get-Content -LiteralPath $map37ePrepScript -Raw
if ($map37ePrepContent -notmatch '\.local') { throw "MAP-37E prepare script missing .local refusal" }
Write-Output "OK: prepare script contains .local refusal language"

Write-Output ""
Write-Output "--- MAP-37E differential control test packet tests ---"
& powershell -ExecutionPolicy Bypass -File $map37eTests
if ($LASTEXITCODE -ne 0) { throw "MAP-37E differential control test packet tests failed." }

Write-Output ""
Write-Output "--- MAP-37D human runtime load-test packet ---"
$map37dDoc        = Join-Path $repoRoot 'docs\MAP_37D_HUMAN_RUNTIME_LOAD_TEST_PACKET.md'
$map37dPrepScript = Join-Path $repoRoot 'scripts\prepare-build42-map37d-human-runtime-load-test-packet.ps1'
$map37dTests      = Join-Path $repoRoot 'scripts\test-build42-map37d-human-runtime-load-test-packet.ps1'
if (-not (Test-Path -LiteralPath $map37dDoc))        { throw "MAP-37D doc missing" }
Write-Output "OK: docs\MAP_37D_HUMAN_RUNTIME_LOAD_TEST_PACKET.md"
if (-not (Test-Path -LiteralPath $map37dPrepScript)) { throw "MAP-37D prepare script missing" }
Write-Output "OK: scripts\prepare-build42-map37d-human-runtime-load-test-packet.ps1"
if (-not (Test-Path -LiteralPath $map37dTests))      { throw "MAP-37D tests missing" }
Write-Output "OK: scripts\test-build42-map37d-human-runtime-load-test-packet.ps1"
$map37dDocContent  = Get-Content -LiteralPath $map37dDoc -Raw
if ($map37dDocContent -notmatch 'MAP37D_HUMAN_RUNTIME_LOAD_TEST_PACKET_DEFINED') { throw "MAP-37D doc missing MAP37D_HUMAN_RUNTIME_LOAD_TEST_PACKET_DEFINED" }
Write-Output "OK: doc contains MAP37D_HUMAN_RUNTIME_LOAD_TEST_PACKET_DEFINED"
if ($map37dDocContent -notmatch 'PLAYABLE_EXPORT_CLAIM_ALLOWED=false') { throw "MAP-37D doc missing PLAYABLE_EXPORT_CLAIM_ALLOWED=false" }
Write-Output "OK: doc contains PLAYABLE_EXPORT_CLAIM_ALLOWED=false"
$map37dPrepContent = Get-Content -LiteralPath $map37dPrepScript -Raw
if ($map37dPrepContent -notmatch '\.local') { throw "MAP-37D prepare script missing .local refusal" }
Write-Output "OK: prepare script contains .local refusal language"

Write-Output ""
Write-Output "--- MAP-37D human runtime load-test packet tests ---"
& powershell -ExecutionPolicy Bypass -File $map37dTests
if ($LASTEXITCODE -ne 0) { throw "MAP-37D human runtime load-test packet tests failed." }
...
$map9bInspectScript   = Join-Path $repoRoot 'scripts\inspect-build42-canary-writer-capability.ps1'
$map9bInspectTests    = Join-Path $repoRoot 'scripts\test-build42-canary-writer-capability.ps1'
$map9bPacketScript    = Join-Path $repoRoot 'scripts\prepare-build42-map9b-canary-writer-unblock-packet.ps1'
$map9bPacketTests     = Join-Path $repoRoot 'scripts\test-build42-map9b-canary-writer-unblock-packet.ps1'
if (-not (Test-Path -LiteralPath $map9bDoc))           { throw "MAP-9B doc missing" }
Write-Output "OK: docs\MAP_9B_CANARY_WRITER_UNBLOCK.md"
if (-not (Test-Path -LiteralPath $map9bInspectScript)) { throw "MAP-9B inspect script missing" }
Write-Output "OK: scripts\inspect-build42-canary-writer-capability.ps1"
if (-not (Test-Path -LiteralPath $map9bInspectTests))  { throw "MAP-9B inspect tests missing" }
Write-Output "OK: scripts\test-build42-canary-writer-capability.ps1"
if (-not (Test-Path -LiteralPath $map9bPacketScript))  { throw "MAP-9B packet script missing" }
Write-Output "OK: scripts\prepare-build42-map9b-canary-writer-unblock-packet.ps1"
if (-not (Test-Path -LiteralPath $map9bPacketTests))   { throw "MAP-9B packet tests missing" }
Write-Output "OK: scripts\test-build42-map9b-canary-writer-unblock-packet.ps1"
$map9bDocContent = Get-Content -LiteralPath $map9bDoc -Raw
if ($map9bDocContent -notmatch 'MAP9B_CANARY_WRITER_UNBLOCK_OUTCOME_B') { throw "MAP-9B doc missing MAP9B_CANARY_WRITER_UNBLOCK_OUTCOME_B" }
Write-Output "OK: doc contains MAP9B_CANARY_WRITER_UNBLOCK_OUTCOME_B"
if ($map9bDocContent -notmatch 'canary_writer_blocked') { throw "MAP-9B doc missing canary_writer_blocked" }
Write-Output "OK: doc contains canary_writer_blocked"
if ($map9bDocContent -notmatch 'PUBLIC_PLAYABLE_CLAIM_ALLOWED=false') { throw "MAP-9B doc missing PUBLIC_PLAYABLE_CLAIM_ALLOWED=false" }
Write-Output "OK: MAP-9B doc contains PUBLIC_PLAYABLE_CLAIM_ALLOWED=false"
if ($map9bDocContent -notmatch 'community_claims_not_adopted_as_doctrine') { throw "MAP-9B doc missing community_claims_not_adopted_as_doctrine" }
Write-Output "OK: MAP-9B doc contains community_claims_not_adopted_as_doctrine"
if ($map9bDocContent -notmatch 'worldmap_bin_playable_terrain_canary_supported=false') { throw "MAP-9B doc missing worldmap_bin_playable_terrain_canary_supported=false" }
Write-Output "OK: MAP-9B doc contains worldmap_bin_playable_terrain_canary_supported=false"
if ($map9bDocContent -notmatch 'DEBUG_RUNTIME_LOGS_REVIEWED=true') { throw "MAP-9B doc missing DEBUG_RUNTIME_LOGS_REVIEWED=true" }
Write-Output "OK: MAP-9B doc contains DEBUG_RUNTIME_LOGS_REVIEWED=true"
$map9bPacketContent = Get-Content -LiteralPath $map9bPacketScript -Raw
if ($map9bPacketContent -notmatch '\.local') { throw "MAP-9B packet script missing .local refusal" }
Write-Output "OK: MAP-9B packet script contains .local refusal language"

Write-Output ""
Write-Output "--- MAP-9B canary writer capability tests ---"
& powershell -ExecutionPolicy Bypass -File $map9bInspectTests
if ($LASTEXITCODE -ne 0) { throw "MAP-9B canary writer capability tests failed." }

Write-Output ""
Write-Output "--- MAP-9B canary writer unblock packet tests ---"
& powershell -ExecutionPolicy Bypass -File $map9bPacketTests
if ($LASTEXITCODE -ne 0) { throw "MAP-9B canary writer unblock packet tests failed." }
...
$psChecks = [ordered]@{
    ...
    'MAP-38B differential control test packet tests (renderable_v1)' = 65
    'MAP-37E differential control test packet tests'              = 60
    'MAP-37D human runtime load-test packet tests'                = 29
...
$psTotal = 2150  # = validation_summary.total_expected_assertions in proof-packet v0.83

$dnCoreTests = 246   # PZMapForge.Core.Tests (MAP-37B: +6 chunkdata record-cluster writer spec tests)
$dnCliTests  = 419   # PZMapForge.Cli.Tests (MAP-38H: +1 RenderableV1 lotpack test, net +1 vs prior 21)
$dnTotal     = 665   # = dotnet_validation_summary.test_total (MAP-38H: +1)

Write-Output ""
Write-Output "  PowerShell lane  (validation_summary in proof-packet v0.83):"
...
Write-Output ""
Write-Output "  .NET lane  (dotnet_validation_summary in proof-packet v0.83 -- tracked separately):"
