...
<#
.SYNOPSIS
    MAP-7D: Generates the empty_grass_v4 candidate (no-BOM text encoding) and
    produces a human-ready load-test packet.
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File scripts\prepare-build42-metadata-v4-load-test-packet.ps1 `
        -Output .local\map7d-packet
#>
...
Write-Output "Generating empty_grass_v4 candidate (no-BOM text encoding)..."
...
$luaMeta = $null
if (Test-Path $luaMetaJson) { $luaMeta = Get-Content $luaMetaJson -Raw | ConvertFrom-Json }
...
# BOM checks (MAP-7D critical: must all be BOM-free)
...
Add-Check 'objects_lua_comment_only_safe' ($objType -eq 'comment_only' -or $objType -eq 'empty' -or $objType -eq 'missing') "objects_type=$objType"
...
if (Test-Path $reportJson) {
    ...
    Add-Check 'report_profile_empty_grass_v4'          ($rep.build42_candidate_profile -eq 'empty_grass_v4') 'profile=empty_grass_v4'
    ...
    foreach ($n in @('report_profile_empty_grass_v4','report_load_tested_false',
...
$preflight = [ordered]@{
    schema                      = 'pzmapforge.map7d-preflight.v0.1'
    map_id                      = $MapId
...
$fence = '```'
$preflightJsonPath = Join-Path $Output 'map7d-preflight.json'
...
$preflightMdPath = Join-Path $Output 'map7d-preflight.md'
$checkTable = ($checks | ForEach-Object { "| $($_.name) | $($_.result) | $($_.detail) |" }) -join "`n"
$preflightMd = @"
# MAP-7D Preflight Report
...
# ---------------------------------------------------------------------------
# Write MAP_7D_LOAD_TEST_PACKET.md
# ---------------------------------------------------------------------------

$packetMdPath = Join-Path $Output 'MAP_7D_LOAD_TEST_PACKET.md'
$packetMd = @"
# MAP-7D Build 42 Candidate No-BOM Encoding Load Test Packet
...
UTF8_BOM_HYPOTHESIS_TEST
PLAYABLE_EXPORT_CLAIM_ALLOWED=false
${fence}

## MAP-7C retest basis and encoding fix

MAP-7C retest (empty_grass_v3) produced LOAD_TEST_FAIL_TIMEOUT_PLAYER_DATA.
- objects.lua still threw LexState.token2str even with comment-only content.
- Inspector confirmed: v3 files had UTF-8 BOM (EF BB BF) prefix.
- Hypothesis: BOM byte (0xEF) interpreted as Lua token ID 65022, overflowing the 31-entry table.
- MAP-7D fix: all game-read text files written with UTF8Encoding(false) — no BOM.

## Preflight status

All $passCount preflight checks PASS.

- Profile:               empty_grass_v4 (MAP-7D)
- LOTH size:             29646 bytes (unchanged from v2/v3)
- LOTH trailer SHA-256:  $trailerSha256
- objects.lua:           comment-only, NO BOM
- spawnpoints.lua:       unemployed key, NO BOM
- mod.info:              NO BOM
- map.info:              NO BOM
...
|---|---|---|
| objects.lua error cleared, spawn passes | LOAD_TEST_PASS or partial | Record and review |
| objects.lua error cleared, timeout again | LOAD_TEST_FAIL_TIMEOUT_PLAYER_DATA | MAP-7E: investigate player-data timeout |
| objects.lua same error (new token) | LOAD_TEST_FAIL_OBJECTS_LUA | MAP-7E: try empty/absent objects.lua |
| New error | LOAD_TEST_INCONCLUSIVE | Record exact error; classify next |
...
${fence}
HUMAN-ONLY: Remove-Item -Recurse -Force 'C:\Users\Palmacede\Zomboid\mods\pzmapforge_build42_candidate_v3_001_test' -ErrorAction SilentlyContinue
HUMAN-ONLY: Remove-Item -Recurse -Force 'C:\Users\Palmacede\Zomboid\mods\$ModFolderName' -ErrorAction SilentlyContinue
${fence}

---

## Step 2: Copy v4 candidate to PZ mods (HUMAN ONLY)
...
See MAP_7D_INSTALL_AND_SERVER_WIRING_COMMANDS.md.
...
function SpawnRegions()
    return {
        {name="PZMapForge v4 Candidate Cell", file="media/maps/$MapId/spawnpoints.lua"},
    }
end
${fence}
...
## Step 4: Patch host.ini, delete stale log (HUMAN ONLY)

${fence}
HUMAN-ONLY: Set in C:\Users\Palmacede\Zomboid\Lua\host.ini: servername=$ServerName
HUMAN-ONLY: Remove-Item -Force '$consoleTxt' -ErrorAction SilentlyContinue
${fence}

---

## Step 5: Run test and capture log (HUMAN ONLY)

1. Launch Build 42, enable only $MapId.
2. Host > select $ServerName.
3. Record in MAP_7D_LOAD_TEST_RECORD.local-template.md.

${fence}
HUMAN-ONLY: Copy-Item '$consoleTxt' '$Output\logs\console-map7d-TIMESTAMP.txt'
${fence}
...
# ---------------------------------------------------------------------------
# Write MAP_7D_LOAD_TEST_RECORD.local-template.md
# ---------------------------------------------------------------------------

$recordMdPath = Join-Path $Output 'MAP_7D_LOAD_TEST_RECORD.local-template.md'
$recordMd = @"
# MAP-7D Load Test Record
...
# ---------------------------------------------------------------------------
# Write MAP_7D_INSTALL_AND_SERVER_WIRING_COMMANDS.md
# ---------------------------------------------------------------------------

$wiringMdPath = Join-Path $Output 'MAP_7D_INSTALL_AND_SERVER_WIRING_COMMANDS.md'
$wiringMd = @"
# MAP-7D Install and Server Wiring Commands
...
${fence}powershell
# HUMAN-ONLY
Remove-Item -Recurse -Force 'C:\Users\Palmacede\Zomboid\mods\pzmapforge_build42_candidate_v3_001_test' -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force 'C:\Users\Palmacede\Zomboid\mods\$ModFolderName' -ErrorAction SilentlyContinue
${fence}

## 2. Copy v4 candidate (HUMAN ONLY)
...
## 5. host.ini and cleanup (HUMAN ONLY)

${fence}powershell
# HUMAN-ONLY
# Set in C:\Users\Palmacede\Zomboid\Lua\host.ini: servername=$ServerName
...
Copy-Item '$consoleTxt' '$Output\logs\console-map7d-TIMESTAMP.txt'
${fence}
"@
...
Write-Output ""
Write-Output "MAP7D_NO_BOM_ENCODING_PACKET_CREATED"
Write-Output "EMPTY_GRASS_V4_CANDIDATE_GENERATED"
Write-Output "OBJECTS_LUA_NO_BOM_FIX_APPLIED"
