...
function Get-NearestKind {
    ...
    foreach ($entry in $PaletteKinds) {
        $distance = Get-ColorDistance -A $Color -B $entry.Color
...
$defaultOutputFull = Resolve-FullPath $defaultOutputDir
...
if (-not $AllowExternalOutput) {
    if (-not (Test-PathInside -Child $outputFull -Parent $defaultOutputFull)) {
...
$requiredKinds = @('grass', 'road', 'sidewalk', 'row_house', 'depanneur', 'garage', 'industrial_yard', 'landmark', 'spawn')
foreach ($required in $requiredKinds) {
    if (-not @($paletteKinds | Where-Object { $_.Kind -eq $required })) {
        throw "Palette missing required kind '$required'."
    }
}
...
try {
    ...
    for ($y = 0; $y -lt $H; $y++) {
        ...
        for ($x = 0; $x -lt $W; $x++) {
            ...
            if ($exactByHex.ContainsKey($upperKey)) {
                ...
                $unmapped[$key]++
                $entry = Get-NearestKind -Color $color -PaletteKinds $paletteKinds
            }
    ...
    Write-TmxFile -Gids $gids -Width $W -Height $H -TileSize $TileSize -TilesetWidth $tilesetWidth -TmxPath $tmxPath
    ...
    if ($unmapped.Count -eq 0) {
        ...
        foreach ($key in ($unmapped.Keys | Sort-Object)) {
            $nearest = Get-NearestKind -Color (ConvertFrom-HexColor -Hex $key) -PaletteKinds $paletteKinds
            $unmappedLines += "- $key -> $($nearest.Kind) ($($unmapped[$key]) pixels)"
        }
    }
...
Generated: deterministic local run (timestamp omitted)

## Claim boundary

Planning artifact only. Not a Project Zomboid load-tested map export. No lotpack, lotheader, or bin files generated.
...
## Unmapped exact colours

$($unmappedLines -join "`n")

## Outputs

| File | Purpose |
|---|---|
| parsed-cell.json | Semantic grid and counts |
...
- Outputs are generated under `.local/mapforge/` by default.
- `media/maps` is not touched.
- No Project Zomboid tilesheets or game assets are copied.
- The TMX uses generated colour tiles for planning only.
"@
    Set-Content -Path $reportPath -Value $report -Encoding UTF8

    if ($Mode -eq 'Debug') {
        Write-Output 'Colour frequencies:'
        foreach ($key in ($colorCounts.Keys | Sort-Object)) {
            Write-Output ("{0} {1}" -f $key, $colorCounts[$key])
        }
        Write-Output 'Unmapped exact colours:'
