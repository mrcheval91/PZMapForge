...
public sealed class DeadMtlWorldBuilderLotSubdivisionPlanProcessTests : IDisposable
{
    private readonly string _tempDir =
        Path.Combine(Path.GetTempPath(), "pzmapforge-lot-plan-cli", Path.GetRandomFileName());

    public DeadMtlWorldBuilderLotSubdivisionPlanProcessTests() =>
    ...
    private static string HelperScript =>
        Path.Combine(RepoRoot, "examples", "deadmtl-layer-pack", "scripts",
            "run-deadmtl-worldbuilder-lot-subdivision-plan.ps1");
    ...
    private (string Json, string Md, string Csv, string Summary) MakeOutputPaths(string subdir = "default")
    {
        ...
        return (
            Path.Combine(dir, "plan.json"),
            Path.Combine(dir, "plan.md"),
            Path.Combine(dir, "plan.csv"),
            Path.Combine(dir, "plan.summary.txt")
        );
    }
    ...
    {
        var (code, _, _) = RunCli("deadmtl-build-worldbuilder-lot-subdivision-plan");
        Assert.Equal(1, code);
    }
    ...
    public void Cli_CreatesOutputJson()
    {
        ...
        RunCli("deadmtl-build-worldbuilder-lot-subdivision-plan",
    ...
    public void Cli_Stdout_ContainsVerdict()
    {
        ...
        Assert.Contains("MAP25C_WORLDBUILDER_LOT_SUBDIVISION_PLAN_CONTRACT_COMPLETE",
    ...
    [Fact]
    public void Cli_UnknownCommand_MentionsLotSubdivisionPlanCommand()
    {
        var (_, stdout, stderr) = RunCli("unknown-xyz-command");
        var combined = stdout + stderr;
        Assert.Contains("deadmtl-build-worldbuilder-lot-subdivision-plan",
    ...
    [Fact]
    public void Doc_Exists_ForLotSubdivisionPlanContract()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_LOT_SUBDIVISION_PLAN_CONTRACT.md");
    ...
    public void Doc_MentionsContractOnly()
    {
        ...
        Assert.True(
            ...
            text.Contains("contract only", StringComparison.OrdinalIgnoreCase),
            "doc should explicitly state CONTRACT ONLY status");
    }
    ...
    public void README_MentionsHelperScript()
    {
        ...
        Assert.Contains("run-deadmtl-worldbuilder-lot-subdivision-plan",
