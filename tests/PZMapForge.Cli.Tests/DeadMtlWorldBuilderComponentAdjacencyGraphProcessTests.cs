...
public sealed class DeadMtlWorldBuilderComponentAdjacencyGraphProcessTests : IDisposable
{
    private readonly string _tempDir =
        Path.Combine(Path.GetTempPath(), "pzmapforge-cag-cli", Path.GetRandomFileName());

    public DeadMtlWorldBuilderComponentAdjacencyGraphProcessTests() =>
    ...
    private static string HelperScript =>
        Path.Combine(RepoRoot, "examples", "deadmtl-layer-pack", "scripts",
            "run-deadmtl-worldbuilder-component-adjacency-graph.ps1");
    ...
    private (string Json, string Md, string Csv, string Summary) MakeOutputPaths(string subdir = "default")
    {
        ...
        return (
            Path.Combine(dir, "cag.json"),
            Path.Combine(dir, "cag.md"),
            Path.Combine(dir, "cag.csv"),
            Path.Combine(dir, "cag.summary.txt")
        );
    }
    ...
    {
        var (code, _, _) = RunCli("deadmtl-build-worldbuilder-component-adjacency-graph");
        Assert.Equal(1, code);
    }
    ...
    public void Cli_CreatesOutputJson()
    {
        ...
        RunCli("deadmtl-build-worldbuilder-component-adjacency-graph",
            "--source-png", SourcePngPath, "--connected-components", ConnectedComponentsPath,
            "--component-intents", ComponentIntentsPath,
            "--geometry-primitive-schema", GeometryPrimitiveSchemaPath,
            "--geometry-preflight", GeometryPreflightPath,
    ...
    public void Cli_Stdout_ContainsVerdict()
    {
        ...
        Assert.Contains("MAP25M_WORLDBUILDER_COMPONENT_ADJACENCY_GRAPH_CONTRACT_COMPLETE",
    ...
    [Fact]
    public void Cli_UnknownCommand_MentionsComponentAdjacencyGraphCommand()
    {
        var (_, stdout, stderr) = RunCli("unknown-xyz-command");
        var combined = stdout + stderr;
        Assert.Contains("deadmtl-build-worldbuilder-component-adjacency-graph",
    ...
    public void README_MentionsHelperScript()
    {
        ...
        Assert.Contains("run-deadmtl-worldbuilder-component-adjacency-graph",
    ...
    public void Doc_Exists()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_COMPONENT_ADJACENCY_GRAPH_CONTRACT.md");
        Assert.True(File.Exists(docPath), $"Doc not found: {docPath}");
    }

    [Fact]
    public void Doc_MentionsContractOnlyAdjacencyGraphNoGenerationNoConcreteGeometry()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_COMPONENT_ADJACENCY_GRAPH_CONTRACT.md");
        ...
        Assert.True(
            text.Contains("adjacency graph", StringComparison.OrdinalIgnoreCase),
            "doc should mention adjacency graph");
        Assert.True(
