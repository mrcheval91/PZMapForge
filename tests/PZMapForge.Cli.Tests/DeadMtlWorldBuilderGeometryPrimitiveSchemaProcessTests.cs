...
public sealed class DeadMtlWorldBuilderGeometryPrimitiveSchemaProcessTests : IDisposable
{
    private readonly string _tempDir =
        Path.Combine(Path.GetTempPath(), "pzmapforge-gps-cli", Path.GetRandomFileName());

    public DeadMtlWorldBuilderGeometryPrimitiveSchemaProcessTests() =>
    ...
    private static string HelperScript =>
        Path.Combine(RepoRoot, "examples", "deadmtl-layer-pack", "scripts",
            "run-deadmtl-worldbuilder-geometry-primitive-schema.ps1");
    ...
    private (string Json, string Md, string Csv, string Summary) MakeOutputPaths(string subdir = "default")
    {
        ...
        return (
            Path.Combine(dir, "gps.json"),
            Path.Combine(dir, "gps.md"),
            Path.Combine(dir, "gps.csv"),
            Path.Combine(dir, "gps.summary.txt")
        );
    }
    ...
    {
        var (code, _, _) = RunCli("deadmtl-build-worldbuilder-geometry-primitive-schema");
        Assert.Equal(1, code);
    }
    ...
    public void Cli_CreatesOutputJson()
    {
        ...
        RunCli("deadmtl-build-worldbuilder-geometry-primitive-schema",
    ...
    [Fact]
    public void Cli_UnknownCommand_MentionsGeometryPrimitiveSchemaCommand()
    {
        var (_, stdout, stderr) = RunCli("unknown-xyz-command");
        var combined = stdout + stderr;
        Assert.Contains("deadmtl-build-worldbuilder-geometry-primitive-schema",
    ...
    public void README_MentionsHelperScript()
    {
        ...
        Assert.Contains("run-deadmtl-worldbuilder-geometry-primitive-schema",
    ...
    public void Doc_Exists()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_GEOMETRY_PRIMITIVE_SCHEMA_CONTRACT.md");
