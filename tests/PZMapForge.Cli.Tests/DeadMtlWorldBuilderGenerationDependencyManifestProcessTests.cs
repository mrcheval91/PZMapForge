...
public sealed class DeadMtlWorldBuilderGenerationDependencyManifestProcessTests : IDisposable
{
    private readonly string _tempDir =
        Path.Combine(Path.GetTempPath(), "pzmapforge-dep-manifest-cli", Path.GetRandomFileName());

    public DeadMtlWorldBuilderGenerationDependencyManifestProcessTests() =>
    ...
    private static string HelperScript =>
        Path.Combine(RepoRoot, "examples", "deadmtl-layer-pack", "scripts",
            "run-deadmtl-worldbuilder-generation-dependency-manifest.ps1");
    ...
    {
        var (code, _, _) = RunCli("deadmtl-build-worldbuilder-generation-dependency-manifest");
        Assert.Equal(1, code);
    }
    ...
    public void Cli_CreatesOutputJson()
    {
        ...
        RunCli("deadmtl-build-worldbuilder-generation-dependency-manifest",
    ...
    [Fact]
    public void Cli_UnknownCommand_MentionsGenerationDependencyManifestCommand()
    {
        var (_, stdout, stderr) = RunCli("unknown-xyz-command");
        var combined = stdout + stderr;
        Assert.Contains("deadmtl-build-worldbuilder-generation-dependency-manifest",
    ...
    public void README_MentionsHelperScript()
    {
        ...
        Assert.Contains("run-deadmtl-worldbuilder-generation-dependency-manifest",
    ...
    public void Doc_Exists()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_GENERATION_DEPENDENCY_MANIFEST_CONTRACT.md");
