...
public sealed class DeadMtlWorldBuilderConnectedComponentExtractionProcessTests : IDisposable
{
    private readonly string _tempDir =
        Path.Combine(Path.GetTempPath(), "pzmapforge-cce-cli", Path.GetRandomFileName());

    public DeadMtlWorldBuilderConnectedComponentExtractionProcessTests() =>
    ...
    private static string HelperScript =>
        Path.Combine(RepoRoot, "examples", "deadmtl-layer-pack", "scripts",
            "run-deadmtl-worldbuilder-connected-component-extraction.ps1");
    ...
    private (string Json, string Md, string Csv, string Summary) MakeOutputPaths(string subdir = "default")
    {
        ...
        return (
            Path.Combine(dir, "cce.json"),
            Path.Combine(dir, "cce.md"),
            Path.Combine(dir, "cce.csv"),
            Path.Combine(dir, "cce.summary.txt")
        );
    }
    ...
    {
        var (code, _, _) = RunCli("deadmtl-build-worldbuilder-connected-component-extraction");
        Assert.Equal(1, code);
    }
    ...
    public void Cli_CreatesOutputJson()
    {
        ...
        RunCli("deadmtl-build-worldbuilder-connected-component-extraction",
    ...
    [Fact]
    public void Cli_UnknownCommand_MentionsConnectedComponentExtractionCommand()
    {
        var (_, stdout, stderr) = RunCli("unknown-xyz-command");
        var combined = stdout + stderr;
        Assert.Contains("deadmtl-build-worldbuilder-connected-component-extraction",
    ...
    public void README_MentionsHelperScript()
    {
        ...
        Assert.Contains("run-deadmtl-worldbuilder-connected-component-extraction",
    ...
    public void Doc_Exists()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_CONNECTED_COMPONENT_EXTRACTION_CONTRACT.md");
        Assert.True(File.Exists(docPath), $"Doc not found: {docPath}");
    }

    [Fact]
    public void Doc_MentionsContractOnlyConnectedComponentNoGenerationNoConcreteGeometry()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_CONNECTED_COMPONENT_EXTRACTION_CONTRACT.md");
        ...
        Assert.True(
            text.Contains("connected component", StringComparison.OrdinalIgnoreCase),
            "doc should mention connected component");
        Assert.True(
