...
public sealed class DeadMtlWorldBuilderNeighborhoodProfileProcessTests : IDisposable
{
    private readonly string _tempDir =
        Path.Combine(Path.GetTempPath(), "pzmapforge-nb-profile-cli", Path.GetRandomFileName());

    public DeadMtlWorldBuilderNeighborhoodProfileProcessTests() =>
    ...
    private static string HelperScript =>
        Path.Combine(RepoRoot, "examples", "deadmtl-layer-pack", "scripts",
            "run-deadmtl-worldbuilder-neighborhood-profile-validation.ps1");
    ...
    private (string Json, string Md, string Csv, string Summary) MakeOutputPaths(string subdir = "default")
    {
        ...
        return (
            Path.Combine(dir, "profile.validation.json"),
            Path.Combine(dir, "profile.validation.md"),
            Path.Combine(dir, "profile.validation.csv"),
            Path.Combine(dir, "profile.validation.summary.txt")
        );
    }

    private (int ExitCode, string Stdout, string Stderr) RunValidate(string? profileOverride = null)
    {
        var (j, m, c, s) = MakeOutputPaths();
        return RunCli(
            "deadmtl-validate-worldbuilder-neighborhood-profile",
            "--profile",     profileOverride ?? BaselineProfile,
            "--output-json", j,
            "--output-md",   m,
            "--output-csv",  c,
            "--summary",     s);
    ...
    [Fact]
    public void Cli_ExitsZero_WithValidBaselineProfile()
    {
    ...
    {
        var (code, _, _) = RunCli("deadmtl-validate-worldbuilder-neighborhood-profile");
        Assert.Equal(1, code);
    }
    ...
    {
        var (j, m, c, s) = MakeOutputPaths("json-test");
        RunCli("deadmtl-validate-worldbuilder-neighborhood-profile",
            "--profile", BaselineProfile,
    ...
    {
        var (j, m, c, s) = MakeOutputPaths("md-test");
        RunCli("deadmtl-validate-worldbuilder-neighborhood-profile",
            "--profile", BaselineProfile,
            "--output-json", j, "--output-md", m, "--output-csv", c, "--summary", s);
    ...
    {
        var (j, m, c, s) = MakeOutputPaths("csv-test");
        RunCli("deadmtl-validate-worldbuilder-neighborhood-profile",
            "--profile", BaselineProfile,
            "--output-json", j, "--output-md", m, "--output-csv", c, "--summary", s);
    ...
    {
        var (j, m, c, s) = MakeOutputPaths("summary-test");
        RunCli("deadmtl-validate-worldbuilder-neighborhood-profile",
            "--profile", BaselineProfile,
            "--output-json", j, "--output-md", m, "--output-csv", c, "--summary", s);
    ...
    [Fact]
    public void Cli_UnknownCommand_MentionsWorldBuilderCommand()
    {
        var (_, stdout, stderr) = RunCli("unknown-command-xyz");
        var combined = stdout + stderr;
        Assert.Contains("deadmtl-validate-worldbuilder-neighborhood-profile",
    ...
    public void Doc_MentionsNoGeneration()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_NEIGHBORHOOD_PROFILE_CONTRACT.md");
        ...
        Assert.True(
            ...
            "doc should explicitly state no generation / contract only");
    }
    ...
    public void README_MentionsHelperScript()
    {
        ...
        Assert.Contains("run-deadmtl-worldbuilder-neighborhood-profile-validation",
