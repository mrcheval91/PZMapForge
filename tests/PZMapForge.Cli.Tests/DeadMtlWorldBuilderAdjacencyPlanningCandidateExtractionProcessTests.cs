...
public sealed class DeadMtlWorldBuilderAdjacencyPlanningCandidateExtractionProcessTests : IDisposable
{
    private readonly string _tempDir =
        Path.Combine(Path.GetTempPath(), "pzmapforge-apce-cli", Path.GetRandomFileName());

    public DeadMtlWorldBuilderAdjacencyPlanningCandidateExtractionProcessTests() =>
    ...
    private static string HelperScript =>
        Path.Combine(RepoRoot, "examples", "deadmtl-layer-pack", "scripts",
            "run-deadmtl-worldbuilder-adjacency-planning-candidate-extraction.ps1");
    ...
    private (string Json, string Md, string Csv, string Summary) MakeOutputPaths(string subdir = "default")
    {
        ...
        return (
            Path.Combine(dir, "apce.json"),
            Path.Combine(dir, "apce.md"),
            Path.Combine(dir, "apce.csv"),
            Path.Combine(dir, "apce.summary.txt")
        );
    }
    ...
    {
        var (code, _, _) = RunCli("deadmtl-build-worldbuilder-adjacency-planning-candidate-extraction");
        Assert.Equal(1, code);
    }
    ...
    public void Cli_CreatesOutputJson()
    {
        ...
        RunCli("deadmtl-build-worldbuilder-adjacency-planning-candidate-extraction",
            "--adjacency-graph", AdjacencyGraphPath,
            "--connected-components", ConnectedComponentsPath,
            "--component-intents", ComponentIntentsPath,
            "--geometry-primitive-schema", GeometryPrimitiveSchemaPath,
    ...
    public void Cli_Stdout_ContainsVerdict()
    {
        ...
        Assert.Contains("MAP25N_WORLDBUILDER_ADJACENCY_PLANNING_CANDIDATE_EXTRACTION_CONTRACT_COMPLETE",
    ...
    [Fact]
    public void Cli_UnknownCommand_MentionsAdjacencyPlanningCandidateExtractionCommand()
    {
        var (_, stdout, stderr) = RunCli("unknown-xyz-command");
        var combined = stdout + stderr;
        Assert.Contains("deadmtl-build-worldbuilder-adjacency-planning-candidate-extraction",
    ...
    public void README_MentionsHelperScript()
    {
        ...
        Assert.Contains("run-deadmtl-worldbuilder-adjacency-planning-candidate-extraction",
    ...
    public void Doc_Exists()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_ADJACENCY_PLANNING_CANDIDATE_EXTRACTION_CONTRACT.md");
        Assert.True(File.Exists(docPath), $"Doc not found: {docPath}");
    }

    [Fact]
    public void Doc_MentionsContractOnlyNoGenerationNoConcreteGeometry()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_ADJACENCY_PLANNING_CANDIDATE_EXTRACTION_CONTRACT.md");
        ...
        Assert.True(
            text.Contains("planning candidate", StringComparison.OrdinalIgnoreCase),
            "doc should mention planning candidate");
        Assert.True(
