...
public sealed class DeadMtlWorldBuilderFutureWorldLayoutPlanProcessTests : IDisposable
{
    private readonly string _tempDir =
        Path.Combine(Path.GetTempPath(), "pzmapforge-fwlp-cli", Path.GetRandomFileName());

    public DeadMtlWorldBuilderFutureWorldLayoutPlanProcessTests() =>
    ...
    private static string HelperScript =>
        Path.Combine(RepoRoot, "examples", "deadmtl-layer-pack", "scripts",
            "run-deadmtl-worldbuilder-future-world-layout-plan.ps1");
    ...
    private (string Json, string Md, string Csv, string Summary) MakeOutputPaths(string subdir = "default")
    {
        ...
        return (
            Path.Combine(dir, "fwlp.json"),
            Path.Combine(dir, "fwlp.md"),
            Path.Combine(dir, "fwlp.csv"),
            Path.Combine(dir, "fwlp.summary.txt")
        );
    }
    ...
    {
        var (code, _, _) = RunCli("deadmtl-build-worldbuilder-future-world-layout-plan");
        Assert.Equal(1, code);
    }
    ...
    public void Cli_CreatesOutputJson()
    {
        ...
        RunCli("deadmtl-build-worldbuilder-future-world-layout-plan",
    ...
    [Fact]
    public void Cli_UnknownCommand_MentionsFutureWorldLayoutPlanCommand()
    {
        var (_, stdout, stderr) = RunCli("unknown-xyz-command");
        var combined = stdout + stderr;
        Assert.Contains("deadmtl-build-worldbuilder-future-world-layout-plan",
    ...
    public void README_MentionsHelperScript()
    {
        ...
        Assert.Contains("run-deadmtl-worldbuilder-future-world-layout-plan",
    ...
    public void Doc_Exists()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_FUTURE_WORLD_LAYOUT_PLAN_CONTRACT.md");
        Assert.True(File.Exists(docPath), $"Doc not found: {docPath}");
    }

    [Fact]
    public void Doc_MentionsContractOnlyAndNoGeneration()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_FUTURE_WORLD_LAYOUT_PLAN_CONTRACT.md");
        ...
        Assert.True(
            ...
            "doc should state no building placement");
    }
}
