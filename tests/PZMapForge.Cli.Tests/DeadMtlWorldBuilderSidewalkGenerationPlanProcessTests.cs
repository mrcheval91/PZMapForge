...
public sealed class DeadMtlWorldBuilderSidewalkGenerationPlanProcessTests : IDisposable
{
    private readonly string _tempDir =
        Path.Combine(Path.GetTempPath(), "pzmapforge-sidewalk-cli", Path.GetRandomFileName());

    public DeadMtlWorldBuilderSidewalkGenerationPlanProcessTests() =>
    ...
    private static string HelperScript =>
        Path.Combine(RepoRoot, "examples", "deadmtl-layer-pack", "scripts",
            "run-deadmtl-worldbuilder-sidewalk-generation-plan.ps1");
    ...
    private (string Json, string Md, string Csv, string Summary) MakeOutputPaths(string subdir = "default")
    {
        ...
        return (
            Path.Combine(dir, "sidewalk_plan.json"),
            Path.Combine(dir, "sidewalk_plan.md"),
            Path.Combine(dir, "sidewalk_plan.csv"),
            Path.Combine(dir, "sidewalk_plan.summary.txt")
        );
    }
    ...
    {
        var (code, _, _) = RunCli("deadmtl-build-worldbuilder-sidewalk-generation-plan");
        Assert.Equal(1, code);
    }
    ...
    public void Cli_CreatesOutputJson()
    {
        ...
        RunCli("deadmtl-build-worldbuilder-sidewalk-generation-plan",
            "--profile", ProfilePath, "--metadata", MetadataPath, "--lot-plan", LotPlanPath,
    ...
    public void Cli_Stdout_ContainsVerdict()
    {
        ...
        Assert.Contains("MAP25D_WORLDBUILDER_SIDEWALK_GENERATION_PLAN_CONTRACT_COMPLETE",
    ...
    [Fact]
    public void Cli_UnknownCommand_MentionsSidewalkPlanCommand()
    {
        var (_, stdout, stderr) = RunCli("unknown-xyz-command");
        var combined = stdout + stderr;
        Assert.Contains("deadmtl-build-worldbuilder-sidewalk-generation-plan",
    ...
    // -----------------------------------------------------------------------
    // Doc and README checks
    // -----------------------------------------------------------------------

    [Fact]
    public void Doc_Exists_ForSidewalkGenerationPlanContract()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_SIDEWALK_GENERATION_PLAN_CONTRACT.md");
        Assert.True(File.Exists(docPath), $"Doc not found: {docPath}");
    }

    [Fact]
    public void Doc_MentionsContractOnly_AndNoGeneration()
    {
        var docPath = Path.Combine(RepoRoot, "docs", "authoring",
            "DEADMTL_WORLDBUILDER_SIDEWALK_GENERATION_PLAN_CONTRACT.md");
        ...
        Assert.True(
            text.Contains("No sidewalk generation", StringComparison.OrdinalIgnoreCase) ||
            text.Contains("no sidewalk generation", StringComparison.OrdinalIgnoreCase),
            "doc should explicitly state no sidewalk generation");
    }
    ...
    public void README_MentionsHelperScript()
    {
        ...
        Assert.Contains("run-deadmtl-worldbuilder-sidewalk-generation-plan",
